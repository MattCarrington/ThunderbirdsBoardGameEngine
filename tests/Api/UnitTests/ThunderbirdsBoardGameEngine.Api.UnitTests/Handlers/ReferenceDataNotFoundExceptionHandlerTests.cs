...
{
    public class ReferenceDataNotFoundExceptionHandlerTests
    {
        [Fact]
        public async Task TryHandleAsync_WhenDisasterCardNotFoundException_ReturnsTrueAsync()
        {
            // Arrange
            var exception = new ReferenceDataNotFoundException("Character", "88");
        ...
        private static ReferenceDataNotFoundExceptionHandler CreateHandler(IProblemDetailsService service)
        {
            var factory = new FakeProblemDetailsFactory();
            var logger = NullLogger<ReferenceDataNotFoundExceptionHandler>.Instance;

            return new ReferenceDataNotFoundExceptionHandler(factory, service, logger);
        }
    }
}
