...
{
    public class PodVehicleCodeTests
    {
        [Fact]
        public void Constructor_WhenValidCode_CreatesInstance()
        {
            // Arrange
            var code = "valid-pod-vehicle-code";

            // Act
            var result = new PodVehicleCode(code);
        ...
        public void Constructor_WhenInvalidCardValues_ThrowsArgumentException(string code)
        {
            ...
            // Act & Assert
            Assert.ThrowsAny<ArgumentException>(() => new PodVehicleCode(code));
        }
    }
}
